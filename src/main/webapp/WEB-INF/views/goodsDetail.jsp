		...
		<div class="top_gnb_area">
			<ul class="list">
				...
				<c:if test="${member != null }">	<!-- 로그인 o -->		
					...
					<li>
						<a href="/cart/${member.userId}">장바구니</a>
					</li>
				</c:if>				
				<li>
					고객센터
		...
		<div class="content_area">
			...
			<div class="content_top">
				<div class="ct_left_area">
					<div class="image_wrap" data-gdsNum="${goodsInfo.imageList[0].gdsNum}" data-path="${goodsInfo.imageList[0].uploadPath}" data-uuid="${goodsInfo.imageList[0].uuid}" data-filename="${goodsInfo.imageList[0].fileName}">
						<img>
					</div>				
				</div>
				<div class="ct_right_area">
					...
					<div class="price">
						<div class="sale_price">정가 : <fmt:formatNumber value="${goodsInfo.gdsPrice}" pattern="#,### 원" /></div>
						<%-- <div class="discount_price">
							판매가 : <span class="discount_price_number"><fmt:formatNumber value="${goodsInfo.gdsPrice - (goodsInfo.bookPrice*goodsInfo.bookDiscount)}" pattern="#,### 원" /></span> 
			...
			<!-- 주문 form -->
			<form action="/order/${member.userId}" method="get" class="order_form">
				<input type="hidden" name="orders[0].gdsNum" value="${goodsInfo.gdsNum}">
				<input type="hidden" name="orders[0].gdsCount" value="">
			</form>	
		</div>
...
$(document).ready(function(){
	...
	const gdsNum = '${goodsInfo.gdsNum}';	

	$.getJSON("/reply/list", {gdsNum : gdsNum}, function(obj){
...
const form = {
		...
		gdsNum : '${goodsInfo.gdsNum}',
		gdsCount : ''
}
// 장바구니 추가 버튼
	$(".btn_cart").on("click", function(e){
		form.gdsCount = $(".quantity_input").val();
		$.ajax({
			url: '/cart/add',
			type: 'POST',
			data: form,
	...
	$(".btn_buy").on("click", function(){
		let gdsCount = $(".quantity_input").val();
		$(".order_form").find("input[name='orders[0].gdsCount']").val(gdsCount);
		$(".order_form").submit();
	});
		...
		$.ajax({
			...
			success : function(result){
				...
				if(result === '1'){
					...
					let popUrl = "/replyEnroll/" + userId + "?gdsNum=" + gdsNum;
					console.log(popUrl);
	...
	/* 리뷰 수정 버튼 */
	 $(document).on('click', '.update_reply_btn', function(e){
			...
			let popUrl = "/replyUpdate?replyId=" + replyId + "&gdsNum=" + '${goodsInfo.gdsNum}' + "&userId=" + '${member.userId}';	
			let popOption = "width = 490px, height=490px, top=300px, left=300px, scrollbars=yes"	
